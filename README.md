...
![alt text](images/SimpleGMPArhictecture.jpg?raw=true "Simple Instrument Architecture"){ width="800" height="600" style="display: block; margin: 0 auto" }
...
If you want to deploy the system in other platform, you should compile the external libraries of the [GIAPI-GLUEc++](https://github.com/framos-gemini/giapi-glue/tree/c++11_Release) for you platform. In the previous link is decribed the instrucction executed by GEMINI to CENTOS7 and ROCKY8.
...
This section describes the instructions for deploying the GMP and [any-command](https://github.com/framos-gemini/giapi-glue/tree/develop/src/examples/any-command.cpp) example implemented in GIAPI-GLUE. The any-command is an example created by GEMINI where developers can see how to implement a command subscriber using the GIAPI-GLUEcc library. Then, we are going to use the giapi-tester to send a command to the any-command example, emulating how any client of the system could send a command to a subscriber. There are more examples in the [src/examples](https://github.com/framos-gemini/giapi-glue/tree/develop/src/examples) folder of the GIAPI-GLUEcc repository. 
